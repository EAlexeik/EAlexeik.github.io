google.maps.__gjsload__('stats', function(_) {
    var OZ = function(a, b, c) {
            var d = [];
            _.Hc(a, function(a, c) {
                d.push(c + b + a)
            });
            return d.join(c)
        },
        PZ = function(a, b, c, d) {
            var e = {};
            e.host = window.document.location && window.document.location.host || window.location.host;
            e.v = a;
            e.r = Math.round(1 / b);
            c && (e.client = c);
            d && (e.key = d);
            return e
        },
        QZ = function(a) {
            var b = {};
            _.Hc(a, function(a, d) {
                b[(0, window.encodeURIComponent)(d)] = (0, window.encodeURIComponent)(a).replace(/%7C/g, "|")
            });
            return OZ(b, ":", ",")
        },
        SZ = function(a, b, c, d) {
            var e;
            e = _.O(_.R, 23, 500);
            var f;
            f = _.O(_.R, 22, 2);
            this.C = a;
            this.D = b;
            this.F = e;
            this.l = f;
            this.B = c;
            this.m = d;
            this.f = new _.Ij;
            this.b = 0;
            this.j = _.Ka();
            RZ(this)
        },
        RZ = function(a) {
            window.setTimeout(function() {
                TZ(a);
                RZ(a)
            }, Math.min(a.F * Math.pow(a.l, a.b), 2147483647))
        },
        UZ = function(a, b, c) {
            a.f.set(b, c)
        },
        TZ = function(a) {
            var b = PZ(a.D, a.B, a.m, void 0);
            b.t = a.b + "-" + (_.Ka() - a.j);
            a.f.forEach(function(a, d) {
                a = a();
                0 < a && (b[d] = Number(a.toFixed(2)) + (_.hm() ? "-if" : ""))
            });
            a.C.b({
                ev: "api_snap"
            }, b);
            ++a.b
        },
        VZ = function(a, b, c, d, e) {
            this.m = a;
            this.C = b;
            this.l = c;
            this.f = d;
            this.j = e;
            this.b = new _.Ij;
            this.B =
            _.Ka()
        },
        WZ = function(a, b, c) {
            this.l = b;
            this.f = a + "/maps/gen_204";
            this.j = c
        },
        XZ = function() {
            this.b = new _.Ij
        },
        YZ = function(a) {
            var b = 0,
                c = 0;
            a.b.forEach(function(a) {
                b += a.Lo;
                c += a.jo
            });
            return c ? b / c : 0
        },
        ZZ = function(a, b, c, d, e) {
            this.B = a;
            this.C = b;
            this.m = c;
            this.j = d;
            this.l = e;
            this.f = {};
            this.b = []
        },
        $Z = function(a, b, c, d) {
            this.j = a;
            _.z.bind(this.j, "set_at", this, this.l);
            _.z.bind(this.j, "insert_at", this, this.l);
            this.B = b;
            this.C = c;
            this.m = d;
            this.f = 0;
            this.b = {};
            this.l()
        },
        a_ = function() {
            this.j = _.P(_.R, 6);
            this.b = new WZ(_.rg[35] ? _.P(_.pf(_.R),
            11) : _.Zv, _.Ti, window.document);
            new $Z(_.Qi, (0, _.p)(this.b.b, this.b), _.ag, !!this.j);
            var a = _.P(new _.jf(_.R.data[3]), 1);
            this.C = a.split(".")[1] || a;
            this.D = {};
            this.B = {};
            this.m = {};
            this.F = {};
            this.G = _.O(_.R, 0, 1);
            _.Si && (this.l = new SZ(this.b, this.C, this.G, this.j))
        };
    VZ.prototype.D = function(a) {
        var b = void 0,
            b = _.m(b) ? b : 1;
        this.b.isEmpty() && window.setTimeout((0, _.p)(function() {
            var a = PZ(this.C, this.l, this.f, this.j);
            a.t = _.Ka() - this.B;
            var b = this.b;
            _.Jj(b);
            for (var e = {}, f = 0; f < b.b.length; f++) {
                var g = b.b[f];
                e[g] = b.H[g]
            }
            _.Wy(a, e);
            this.b.clear();
            this.m.b({
                ev: "api_maprft"
            }, a)
        }, this), 500);
        b = this.b.get(a, 0) + b;
        this.b.set(a, b)
    };
    WZ.prototype.b = function(a, b) {
        b = b || {};
        var c = _.yk().toString(36);
        b.src = "apiv3";
        b.token = this.l;
        b.ts = c.substr(c.length - 6);
        a.cad = QZ(b);
        a = OZ(a, "=", "&");
        a = this.f + "?target=api&" + a;
        this.j.createElement("img").src = a;
        (b = _.Mc.__gm_captureCSI) && b(a)
    };
    XZ.prototype.f = function(a, b, c) {
        this.b.set(_.xb(a), {
            Lo: b,
            jo: c
        })
    };
    ZZ.prototype.D = function(a) {
        this.f[a] || (this.f[a] = !0, this.b.push(a), 2 > this.b.length && _.kz(this, this.F, 500))
    };
    ZZ.prototype.F = function() {
        for (var a = PZ(this.C, this.m, this.j, this.l), b = 0, c; c = this.b[b]; ++b)
            a[c] = "1";
        a.hybrid = +_.gl();
        this.b.length = 0;
        this.B.b({
            ev: "api_mapft"
        }, a)
    };
    $Z.prototype.l = function() {
        for (var a; a = this.j.removeAt(0);) {
            var b = a.Mn;
            a = a.timestamp - this.C;
            ++this.f;
            this.b[b] || (this.b[b] = 0);
            ++this.b[b];
            if (20 <= this.f && !(this.f % 5)) {
                var c = {};
                c.s = b;
                c.sr = this.b[b];
                c.tr = this.f;
                c.te = a;
                c.hc = this.m ? "1" : "0";
                this.B({
                    ev: "api_services"
                }, c)
            }
        }
    };
    a_.prototype.S = function(a) {
        a = _.xb(a);
        this.D[a] || (this.D[a] = new ZZ(this.b, this.C, this.G, this.j));
        return this.D[a]
    };
    a_.prototype.O = function(a) {
        a = _.xb(a);
        this.B[a] || (this.B[a] = new VZ(this.b, this.C, 1, this.j));
        return this.B[a]
    };
    a_.prototype.f = function(a) {
        if (this.l) {
            this.m[a] || (this.m[a] = new _.kA, UZ(this.l, a, function() {
                return b.nb()
            }));
            var b = this.m[a];
            return b
        }
    };
    a_.prototype.ea = function(a) {
        if (this.l) {
            this.F[a] || (this.F[a] = new XZ, UZ(this.l, a, function() {
                return YZ(b)
            }));
            var b = this.F[a];
            return b
        }
    };
    var b_ = new a_;
    _.lc("stats", b_);
});
